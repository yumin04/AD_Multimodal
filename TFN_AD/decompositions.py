...
def tucker_decomposition_conv_layer(layer):
    ...
    # (core, factors), rec_errors = partial_tucker(layer.weight.data, modes=[0, 1], rank=ranks, init='svd')
    (core, [last, first]), rec_errors = \
        partial_tucker(layer.weight.data, \
            modes=[0, 1], rank=ranks, init='svd')
    print("Decomposed Shape:", core.shape, last.shape, first.shape)
    ...
    if False:
        ...
        norm_tensor = tl.norm(layer.weight.data, 2)
        print((norm_rec - norm_tensor) / norm_rec < tol_norm_2)

        # Test the max abs difference between the reconstruction and the tensor
        print(tl.max(tl.abs(norm_rec - norm_tensor)) < tol_max_abs)
